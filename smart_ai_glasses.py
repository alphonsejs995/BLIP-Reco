"""
Smart AI Glasses - Complete System
Run: python smart_glasses.py
...
class Config:
    ...
    # Timing
    OBJECT_INTERVAL: float = 2.0  # Detect objects every 2 sec
    TEXT_INTERVAL: float = 5.0    # Detect text every 5 sec
    MIN_REPEAT_TIME: float = 15.0 # Don't repeat same object within 15 sec
...
class OCRReader:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            gpu_available = False  # We'll test this
...
class SmartGlassesSystem:
        ...
        while self.running:
            ...
            if current_time - self.last_object_time >= self.config.OBJECT_INTERVAL:
                ...
                for detection in detections:
                    if detection['confidence'] > 0.6:  # High confidence threshold for speech
                        # Calculate spatial information
                        spatial_info = self.spatial.analyze(detection, frame.shape[1])
                        ...
                        if self.config.SHOW_BBOX:
                            frame = self.visualizer.draw_object(frame, detection, spatial_info)
            ...
            if current_time - self.last_text_time >= self.config.TEXT_INTERVAL:
                ...
                if texts:
                    ...
                    if best_text['confidence'] > 0.6:
                        ...
                        # Visualize
                        if self.config.SHOW_BBOX:
                            frame = self.visualizer.draw_text_region(frame, best_text)
                        
                        text_count = len(texts)
            
            # === DISPLAY ===
            # Add debug information
            frame = self.visualizer.draw_debug_info(
                frame, fps, 
            ...
            # Show frame
            cv2.imshow("Smart AI Glasses", frame)
    ...
    def test_all_components(self):
        ...
        if self.ocr.ocr_loaded:
            ...
            if texts:
                for text in texts[:2]:  # Show first 2
